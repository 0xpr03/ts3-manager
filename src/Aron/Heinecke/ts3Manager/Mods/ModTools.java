...
public class ModTools implements Mod {
	...
	private void rocketchan(String target, int applicant, boolean ignore_clients) {
		...
		try {
			...
			HashMap<String, String> tmap = ts3conn.getConnector().getInfo(JTS3ServerQuery.INFOMODE_CLIENTINFO, tid); // target
																														// infos
			...
			if (tid != applicant)
				ts3conn.getConnector().sendTextMessage(applicant, JTS3ServerQuery.TEXTMESSAGE_TARGET_CLIENT,
						"[b]Client " + tmap.get("client_nickname") + "[" + tmap.get("client_database_id")
								+ "] was rocketed![/b]");
	...
	public ModRegisters registerEvents() {
		return new ModRegisters.Builder().eventTextChannel(true).eventTextPrivate(true).eventTextServer(true).build();
	}
